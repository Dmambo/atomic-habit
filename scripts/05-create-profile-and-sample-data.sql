-- Create profile and sample data for user: ibkhalidworld@gmail.com
-- This script handles the case where the profile wasn't automatically created
...
BEGIN
    ...
    -- Create or update the profile (in case it doesn't exist)
    INSERT INTO public.profiles (id, email, full_name, created_at, updated_at)
    VALUES (
        user_uuid, 
        'ibkhalidworld@gmail.com', 
        'Diaby Ibrahim',
        NOW(),
        NOW()
    )
    ...
    -- Delete any existing data for this user to avoid conflicts
    ...
    -- Insert sample goals and capture their IDs
    INSERT INTO public.goals (user_id, title, description, category, color, target_date, motivation_note) 
    VALUES (user_uuid, 'Fitness & Health', 'Build a strong and healthy body through consistent exercise and healthy habits', 'Health', 'from-blue-500 to-green-500', '2024-12-31', 'Every workout makes me stronger and more confident!')
    RETURNING id INTO goal_fitness_uuid;
    
    INSERT INTO public.goals (user_id, title, description, category, color, target_date, motivation_note) 
    VALUES (user_uuid, 'Learning & Growth', 'Expand knowledge and skills through continuous learning', 'Education', 'from-purple-500 to-pink-500', '2024-11-30', 'Knowledge is power, and I''m building my future!')
    RETURNING id INTO goal_learning_uuid;
    
    INSERT INTO public.goals (user_id, title, description, category, color, target_date, motivation_note) 
    VALUES (user_uuid, 'Mindfulness', 'Cultivate inner peace and awareness through meditation and reflection', 'Wellness', 'from-green-500 to-teal-500', '2024-10-31', 'Peace comes from within. I am creating my sanctuary.')
    RETURNING id INTO goal_mindfulness_uuid;
    
    INSERT INTO public.goals (user_id, title, description, category, color, target_date, motivation_note) 
    VALUES (user_uuid, 'Productivity', 'Optimize work and personal efficiency through better systems', 'Work', 'from-orange-500 to-red-500', '2024-12-15', 'Small improvements compound into extraordinary results.')
    RETURNING id INTO goal_productivity_uuid;
    
    RAISE NOTICE 'Created % goals', 4;
    ...
    RAISE NOTICE 'Created % habits', 12;
    ...
    WHERE h.user_id = user_uuid 
      AND h.name IN ('Morning workout', 'Morning meditation', 'Read for 30 minutes', 'Take vitamins')
      AND random() > 0.15; -- 85% completion rate for sample data
    
    -- Add some more varied completion data
    INSERT INTO public.habit_completions (user_id, habit_id, completed_date)
    SELECT 
        user_uuid,
        h.id,
    ...
    FROM public.habits h
    WHERE h.user_id = user_uuid 
      AND h.name IN ('Drink 8 glasses of water', 'Evening walk', 'Practice coding')
      AND random() > 0.3; -- 70% completion rate for these habits
    
    RAISE NOTICE 'Added sample habit completions';
